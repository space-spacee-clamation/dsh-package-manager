...
function handle(ctx, service, apiPrefix) {
	return async (req, res) => {
		...
		try {
			...
			if (req.method === "GET" && path === "/local-plugins") return send(res, 200, {
				ok: true,
				value: service.localPlugins()
			});
			if (req.method === "POST" && path === "/local-plugins/refresh") return dispatch(res, req, () => Promise.resolve(service.refreshLocalPlugins()));
			if (req.method === "POST" && path === "/local-plugins/release") return dispatch(res, req, (body) => service.releaseLocalPlugin(body));
			if (req.method === "POST" && path === "/local-plugins/restore") return dispatch(res, req, (body) => service.restoreLocalPlugin(body));
			...
			if (req.method === "POST" && path === "/check-update") return dispatch(res, req, (body) => service.checkUpdate(body));
