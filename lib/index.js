import { a as defaultWorkspaceRoot, c as packageStoreRoot, d as resolveHome, f as runtimeRoot, l as pmRoot, m as workspaceHistoryPath, n as pluginEnvDir, o as isAbsolutePath, p as workRoot, r as pluginWorkspaceDir, s as ledgerPath, t as ensurePluginWorkspace, u as profileDir } from "./pluginWorkspace-CmSq_H7F.js";
...
var LocalPluginRegistry = class {
	...
	constructor(ctx) {
		this.ctx = ctx;
		for (const runtime of ctx.registry.values()) for (const fiber of runtime.fibers) this.observe(fiber);
...
function launchPath(home) {
	return join(runtimeRoot(home), LAUNCH_FILENAME);
}
function portOf(env = process.env) {
...
* script arguments.
*/
function recordLaunch(home, port, argv = process.argv.slice(1), cwd = process.cwd()) {
	const args = [...argv];
	const record = {
		argv: args,
		cwd,
...
function readLaunch(home) {
	...
	try {
		...
		const raw = parsed;
		if (typeof raw.command !== "string" || raw.command === "") return void 0;
		return {
			argv: Array.isArray(raw.argv) ? raw.argv.filter((item) => typeof item === "string") : [],
...
*/
function buildRestartScript(port, command, logPath) {
	return [
...
*/
function spawnRestarter(script) {
...
var PackageManagerService = class extends Service {
	...
	*/
	restart(command) {
		...
		const cmd = command !== void 0 && command !== "" ? command : record?.command ?? "";
		if (cmd === "") throw new Error("no restart command recorded and none provided — pass a command");
		...
		return {
			...
			command: cmd,
			log,
			pid: spawnRestarter(buildRestartScript(port, cmd, log)),
			port
		};
	}
	doctor() {
