...
async function ensureGitMirror(mirror, url, git) {
...
var CordisRuntime = class {
	ctx;
	loader;
	loaderEntries = /* @__PURE__ */ new Map();
	...
	findLoaderEntry(packageName) {
		if (this.loader === void 0) return void 0;
	...
	async mount(entry) {
		...
		if (this.loader !== void 0) return this.mountNative(entry);
	...
	async mountNative(entry) {
		...
		try {
			...
			if (existing !== void 0) {
				...
				return {
					mounted: true,
					reason: "already present in the native Cordis loader tree"
				};
			}
			...
			return {
				mounted: true,
				reason: "mounted through ctx.loader (native HMR lifecycle)"
			};
		} catch (error) {
			return {
				mounted: false,
	...
	async unmountNative(entry) {
		try {
			...
			if (target === void 0) target = this.findLoaderEntry(entry.packageName);
			if (target === void 0) return {
				mounted: false,
				reason: `${entry.packageName} has no native loader entry`
			};
			...
			return {
				mounted: true,
				reason: "removed from ctx.loader (native HMR lifecycle)"
			};
		} catch (error) {
			return {
				mounted: false,
	...
	async unmountFallback(entry) {
		...
		try {
			...
			if (owned !== void 0) {
				...
				return {
					mounted: true,
					reason: "disposed ctx.plugin fallback fiber"
				};
			}
			const plugin = pluginOf(await this.importPlugin(entry));
