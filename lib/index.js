...
var LocalPluginRegistry = class {
	...
	refresh() {
		...
		for (const [name, entry] of this.loaderEntries()) {
			...
			if (fiber !== void 0) this.observeFiber(fiber, name);
	...
	async restore(name) {
		...
		if (entry !== void 0) {
			...
			this.cacheRow(name, entry.fiber ?? null, entry.fiber?.state === 2, false, entry.fiber === void 0);
			return this.cache.get(name);
		}
		const restored = [];
		for (const record of this.records.get(name) ?? []) {
		...
		const info = restored.length > 0 ? this.infoOf(name, restored[0].fiber, false, false) : {
			name,
			uid: null,
			state: 0,
			active: false,
	...
	observeFiber(fiber, name) {
		...
		if (record === void 0) {
			record = {
				...
				config: fiber._config,
				fiber
			};
			bucket.push(record);
			this.records.set(name, bucket);
		} else record.fiber = fiber;
		this.cacheRow(name, fiber, fiber.state === 2, false, false);
	}
	cacheRow(name, fiber, active, released, cached) {
	...
	infoOf(name, fiber, released, cached) {
		return {
			name,
			uid: fiber.uid,
			state: fiber.state,
	...
	loaderNameOf(fiber) {
		for (const [name, entry] of this.loaderEntries()) if (entry.fiber === fiber) return name;
	}
};
//#endregion
//#region src/selfMigration.ts
/**
...
*/
function migratePackageManagerSelf(home, packageManagerDir, profile = "web") {
	const dir = profileDir(home, profile);
	const managed = managedPatchForPackage(packageManagerDir, profile, "package-manager");
	if (managed === void 0) throw new Error(`package-manager bundle patch not found in ${packageManagerDir}`);
...
var PackageManagerService = class extends Service {
	...
	releaseLocalPlugin(request) {
		return this.localPluginRegistry.release(request.name);
	}
	/** Restore a previously released local plugin. */
	restoreLocalPlugin(request) {
...
//#endregion
export { Config, HarnessHost, LocalHost, PackageManager, PackageManagerService, apply, createHost, createPackageManager, idFromSource, migratePackageManagerSelf, name };
